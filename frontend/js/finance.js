/**
 * finance.js — الكود الخاص بلوحة تحكم المدير المالي
...
function enforceRoleAccess(requiredRole) {
    ...
    if (!user.role) return;
    // استثناء خاص: manager_finance يمكنه الوصول إلى واجهة المالية
    if (user.role === 'manager' && user.username === 'manager_finance') return;
...
async function loadRequests() {
    try {
        ...
        // تحديث الإحصائيات من البايلود إن توفرت
...
function updateStats() {
    ...
    if (user.username === 'manager_finance') {
        pending = allRequests.filter(r => {
            ...
            const isFinanceDept = r.department === 'مالية';
            return (status === 'pending_manager' && isFinanceDept) || status === 'pending_finance';
        }).length;
    } else {
        pending = allRequests.filter(r => (r.status || 'pending_manager') === 'pending_finance').length;
    }
      ...
      <td>
        ...
        ${request.status !== 'approved' && request.status !== 'rejected' ? `
          <button class="action-btn btn-approve" onclick="quickUpdateStatus(${request.id}, 'approve')">
            <i class="fas fa-check"></i> موافقة مالية
          </button>
...
function renderRejectedRequestsTable(requestsToRender = null) {
    ...
    if (requests.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center">لا توجد طلبات مرفوضة حالياً.</td></tr>`;
    }
}
...
async function viewRequest(requestId) {
    ...
    const canAct = request.status !== 'approved' && request.status !== 'rejected';
    ...
    <div class="items-list">
      ...
      <table class="items-table-modal">
        <thead><tr>
          ...
          <th>السعر</th><th>المجموع</th><th>الحالة</th>${canAct ? '<th>الإجراءات</th>' : ''}
        </tr></thead>
        <tbody>
          ${request.items ? request.items.map(item => `
            <tr id="item-row-${item.id}" ${item.status === 'rejected' ? 'style="background:#fff5f5"' : ''}>
              ...
              ${canAct ? `<td>${(!item.status || item.status === 'pending') ? `
                <div style="display:flex;gap:5px;">
                  <button onclick="approveItem(${request.id},${item.id})" style="background:#2ecc71;color:white;border:none;border-radius:4px;padding:2px 5px;cursor:pointer;" title="موافقة">✓</button>
                  <button onclick="rejectItem(${request.id},${item.id})" style="background:#e74c3c;color:white;border:none;border-radius:4px;padding:2px 5px;cursor:pointer;" title="رفض">✗</button>
                </div>
              ` : (item.rejected_by ? `<small>رفض: ${item.rejected_by}</small>` : '')}</td>` : ''}
            </tr>
          `).join('') : '<tr><td colspan="8">لا توجد مواد</td></tr>'}
        </tbody>
        <tfoot><tr style="font-weight:bold;background:#f8f9fa;">
          <td colspan="5">المجموع الكلي</td>
          <td colspan="${canAct ? 3 : 2}">${formattedTotal}</td>
        </tr></tfoot>
      </table>
    </div>
  `;
...
async function updateStatus(action, note = '', signature = null) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            alert(`تم ${action === 'approve' ? 'الموافقة المالية على' : 'رفض'} الطلب بنجاح`);
            closeModal();
            // تحديث الكاردات إن جاءت مع الاستجابة
...
function showPendingRequests() {
    ...
    document.querySelector('.page-title').textContent = 'طلبات الشراء - المدير المالي';
...
function calculateFinanceReports(requests) {
    ...
    updateFinanceCharts(deptCounts, requests);
    updateFinanceMonthlyStats(requests);
}

function updateFinanceCharts(departmentCounts, requests) {
...
function updateFinanceMonthlyStats(requests) {
...
async function approveItem(requestId, itemId) {
    try {
        ...
        if (res.ok) {
            ...
            if (result.request && result.request.total_amount !== undefined) updateTotalAmountUI(result.request.total_amount);
...
function updateItemUI(itemId, status, reason = '') {
    ...
    if (!row) return;
    row.cells[6].innerHTML = `${getItemStatusBadge(status)} ${reason ? '<br><small style="color:red">' + reason + '</small>' : ''}`;
    row.cells[7].innerHTML = status === 'rejected' ? `<small>مرفوض</small>` : '';
    row.style.backgroundColor = status === 'rejected' ? '#fff5f5' : '';
}

function updateTotalAmountUI(newTotal) {
    const footerTotal = document.querySelector('.items-table-modal tfoot td:last-child');
    if (footerTotal) {
        let currency = footerTotal.textContent.includes('ل.س') ? 'SYP' : 'USD';
        footerTotal.innerHTML = formatCurrency(newTotal, currency);
    }
}
